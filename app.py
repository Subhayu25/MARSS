...
@st.cache_data
def load_campaign():
    xl = pd.ExcelFile("PBL_GJ25NS020_DAIDM.xlsx")
    df = xl.parse("marketing_campaign_dataset", parse_dates=["Date"])
    return df

# Load data
survey_df = load_survey()
campaign_df = load_campaign()
...
with st.sidebar.expander("Campaign Data Filters", expanded=True):
    chan_sel = st.multiselect("Channel",      campaign_df["Channel_Used"].unique(),     default=campaign_df["Channel_Used"].unique())
    ctype_sel= st.multiselect("Campaign Type",campaign_df["Campaign_Type"].unique(),    default=campaign_df["Campaign_Type"].unique())
    dmin = campaign_df["Date"].min().date()
    dmax = campaign_df["Date"].max().date()
...
campaign_mask = (
    campaign_df["Channel_Used"].isin(chan_sel) &
    campaign_df["Campaign_Type"].isin(ctype_sel) &
    (campaign_df["Date"].dt.date >= date_range[0]) &
    (campaign_df["Date"].dt.date <= date_range[1])
)
camp_f = campaign_df[campaign_mask].copy()
...
with tabs[0]:
    ...
    st.markdown("""
    Welcome!  
    - **Survey**: identify drivers of individual conversion & ROI proxy.  
    - **Campaign**: replicate your Excel charts for deep campaign insights.  
...
with tabs[2]:
    ...
    fig = px.bar(conv_seg, x=seg_dim, y="Purchase_Last_3mo",
                 labels={"Purchase_Last_3mo":"Conv. Rate"}, text=conv_seg["Purchase_Last_3mo"].map("{:.1%}".format))
...
with tabs[4]:
    ...
    # Bar: Conversion Rate by Channel
...
with tabs[5]:
    ...
    # Correlation heatmap (survey)
    st.subheader("Survey Feature Correlations")
    num = survey_f.select_dtypes(include="number").corr()
    fig11, ax = plt.subplots(figsize=(6,5))
    sns.heatmap(num, annot=True, fmt=".2f", cmap="coolwarm", ax=ax)
    st.pyplot(fig11)

    # Correlation heatmap (campaign)
    st.subheader("Campaign Metric Correlations")
    corr2 = camp_f.select_dtypes(include="number")[sel_cols].corr()
    fig12, ax2 = plt.subplots(figsize=(6,5))
    sns.heatmap(corr2, annot=True, fmt=".2f", cmap="coolwarm", ax=ax2)
    st.pyplot(fig12)
